...
async function main() {
  ...
  // Payments: only whoever recorded one may delete it
  ...
  check("the other home cannot delete someone else's payment", stillSet?.length === 1);
  await parent.client.from('settlements').delete().eq('id', setId);
  const { data: setAfter } = await admin.from('settlements').select('id').eq('id', setId);
  check('the creator can delete their own payment', (setAfter || []).length === 0);
