...
// "A", "A & B", "A, B & C" — reads as prose at any length.
...
// Does an activity occur on this date? One-day activities (start = end) always
// count; recurring ones match the season range and weekday list.
export function activityOn(act, dateStr) {
  if (dateStr < act.start_date || dateStr > act.end_date) return false;
  if (act.start_date === act.end_date) return true;
  return (act.days || []).includes(pd(dateStr).getDay());
}

// Positive = household side owes co-parent. Only approved expenses count.
// The h-side share of one expense: its own override, else the arrangement's.
export function expenseSplit(arrangement, e) {
  return (e.split_pct ?? arrangement.split_pct) / 100;
}
...
export function balance(arrangement, expenses, settlements) {
  ...
  for (const e of expenses) {
    ...
    const split = expenseSplit(arrangement, e);
