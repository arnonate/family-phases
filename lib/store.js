...
import { listNames } from '@/lib/custody';
...
export function StoreProvider({ children }) {
  const [state, setState] = useState({ loading: true, me: null, households: [], arrangements: [], notifications: [], invites: [], dismissed: [] });
  ...
  const refresh = useCallback(async () => {
    ...
    const [profileQ, houseQ, arrQ, notifQ, inviteQ, prefsQ] = await Promise.all([
      ...
      s.from('arrangements').select(`*,
        ...
        posts(*, profiles(id,name,email), post_comments(*, profiles(id,name,email)))`),
      s.from('notifications').select('*').order('created_at', { ascending: false }).limit(50),
      // RLS returns only invites I sent (or ones addressed to me)
      s.from('invites').select('*').eq('claimed', false).order('created_at'),
      s.from('user_prefs').select('dismissed'),   // RLS: own row only
    ]);
    ...
    const arrangements = (arrQ.data || []).map(a => ({
      ...
      posts: (a.posts || [])
        .map(p => ({
          ...p,
          comments: (p.post_comments || []).sort((x, y) => (x.created_at < y.created_at ? -1 : 1)),
        }))
        // newest activity first: latest reply, else the post itself
        .sort((x, y) => {
          const lx = x.post_comments?.length ? x.post_comments[x.post_comments.length - 1].created_at : x.created_at;
          const ly = y.post_comments?.length ? y.post_comments[y.post_comments.length - 1].created_at : y.created_at;
          return lx < ly ? 1 : -1;
        }),
      todos: (a.todos || [])
        .map(t => ({
    ...
    setState({
      ...
      invites: inviteQ.data || [],
      dismissed: prefsQ.data?.[0]?.dismissed || [],
    });
  }, []);

  // Permanently dismiss a UI element for this account (optimistic).
  const dismiss = useCallback(async key => {
    setState(v => ({ ...v, dismissed: [...new Set([...v.dismissed, key])] }));
    const s = supa();
    const { data: { user } } = await s.auth.getUser();
    if (!user) return;
    const { data } = await s.from('user_prefs').select('dismissed').eq('user_id', user.id);
    const cur = data?.[0]?.dismissed || [];
    await s.from('user_prefs').upsert({ user_id: user.id, dismissed: [...new Set([...cur, key])] });
  }, []);
  ...
  return <Ctx.Provider value={{ ...state, refresh, dismiss }}>{children}</Ctx.Provider>;
}
...
export function arrName(arr) {
  ...
  if (kids.length) return listNames(kids);
  return arr.name;
}
