  ...
  const refresh = useCallback(async () => {
    ...
    const [profileQ, houseQ, arrQ, notifQ] = await Promise.all([
      ...
      s.from('arrangements').select(`*,
        ...
        children(*), schedules(*), deviations(*), expenses(*), settlements(*), todos(*)`),
    ...
    const arrangements = (arrQ.data || []).map(a => ({
      ...
      todos: (a.todos || []).sort((x, y) => ((x.due || '9999') < (y.due || '9999') ? -1 : 1)),
    }));
...
export function sideName(arr, side) {
  ...
  if (side === 'h') return arr.h_label || 'Household parent';
  return arr.c_label || 'Co-parent';
}
